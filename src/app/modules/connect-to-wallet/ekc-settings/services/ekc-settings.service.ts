...
export class EkcSettingsService {
  ...
  edit() {
    this.dialog.open(EkcSettingsComponent, {
      width: '600px',
      data: {
        url: this.url,
      }
    }).afterClosed().pipe(truthy()).subscribe((url) => {
      this.saveToStorage = url;
    });
  }

  get url(): string {
    return localStorage.getItem(EKC_STORAGE_URL) ? localStorage.getItem(EKC_STORAGE_URL) : this.envService.ekcUrl;
  }
